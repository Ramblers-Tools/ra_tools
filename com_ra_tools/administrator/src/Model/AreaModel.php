 ...
 * 22/02/25 CB copied working version back from test
 */
...
class AreaModel extends AdminModel {
    ...
    protected function loadFormData() {
        ...
        $data = Factory::getApplication()->getUserState('com_ra_tools.edit.area.data', array());
    ...
    public function duplicate(&$pks) {
        ...
        foreach ($pks as $pk) {
            ...
            if ($table->load($pk, true)) {
                ...
                if (!empty($table->nation_id)) {
                    if (is_array($table->nation_id)) {
    ...
    protected function prepareTable($table) {
        ...
        if (empty($table->id)) {
            ...
            if (@$table->ordering === '') {
                $db = $this->getDbo();
                $db->setQuery('SELECT MAX(ordering) FROM #__ra_areas');
                $max = $db->loadResult();
