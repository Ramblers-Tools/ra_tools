 ...
 * 21/04/25 CB replace Ra_toolsHelper with ToolsHelper
 * 03/05/25 CB delete code for attachments
 */
 ...
 * @since 3.0.0
 */
class NationTable extends Table implements VersionableTableInterface, TaggableTableInterface {
    ...
    public function __construct(DatabaseDriver $db) {
        $this->typeAlias = 'com_ra_tools.nation';
        parent::__construct('#__ra_nations', 'id', $db);
    ...
    public function bind($array, $ignore = '') {
        ...
        if (!$user->authorise('core.admin', 'com_ra_tools.nation.' . $array['id'])) {
            $actions = Access::getActionsFromFile(
                            JPATH_ADMINISTRATOR . '/components/com_ra_tools/access.xml',
                            "/access/section[@name='nation']/"
            );
            $default_actions = Access::getAssetRules('com_ra_tools.nation.' . $array['id'])->getData();
            $array_jaccess = array();
    ...
    public function delete($pk = null) {
        ...
        if ($result) {
            ...
            switch ($checkImageVariableType) {
                case 'string':
                    File::delete(JPATH_ROOT . '/com_ra_tools/' . $this->attachment);
                    break;
                default:
                    foreach ($this->attachment as $attachmentFile) {
                        File::delete(JPATH_ROOT . '/com_ra_tools/' . $attachmentFile);
                    }
            }
        }
