...
$target = 'administrator/index.php?option=com_ra_tools&task=area.showArea&code=';
...
if (empty($this->items)) {
    ...
    foreach ($this->items as $i => $item) {
        ...
        if ($this->canDo->get('core.edit')) {
            echo $objHelper->buildLink($target . $item->code, $item->name, False, "");
            //          echo '<a class="hasTooltip" href="' . Route::_('index.php?option=com_ra_tools&view=area&layout=edit&id=' . $item->id) . '">' . PHP_EOL;
        ...
        if ($group_count > 0) {
            ...
            echo '<a href="' . Route::_('index.php?option=com_ra_tools&task=area.showGroups&area=' . $item->code) . '">' . PHP_EOL;
